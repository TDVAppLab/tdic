...
//    const navigate = useNavigate();
    ...
    function handleFormSubmitResetAnimationClipArray() {
        ...
        if(id){
            ...
            agent.Modelfiles.updatePartAnimationClip(id[0],PartAnimationClips).then((response) => {});
            ...
            <Row>
                <Col  sm={6} >
                    <div className="row" style={{ height:"45vh", width:'45vw' }}>                            
                        ...
                        >
                            <ModelfileViewer id_part={id[0]} setTeststring={setAnimations} setModelUuid = {setModelUuid}/>
                    ...
                    <Tabs defaultActiveKey="modelfile_dataedit" id="modelfile-tab-edit" >
                        <Tab eventKey="modelfile_dataedit" title="Edit">
                            ...
                            <Formik
                                ...
                                initialValues={modelfile} 
                                onSubmit={values => {
                                    deleteModelfile(values).then(()=>{ redirect(`/modelfiles`) }) 
                                }}>
                                {({ handleSubmit, isValid, isSubmitting, dirty }) => (
                                    <Form className="ui form" onSubmit = {handleSubmit} autoComplete='off'>
                                    ...
                                    <tbody>
                                        {
                                            ...
                                            selectedModelfile && selectedModelfile.article_references.map(x=>(
                                                <tr key={x.id_article}>
                                                    <td>{x.id_article}</td>
                                                    <td><Link href={`/articleedit/${x.id_article}`}>{x.title}</Link></td>
                                                    <td>{x.status_name}</td>
                                                </tr>
                                            ))
                                        }
